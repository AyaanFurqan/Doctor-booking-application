        ...
        <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
            ...
            <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
                {doctors.slice(0, 10).map((item, index) => (
                    <div onClick={()=>{ item.available ? navigate(`/appointment/${item._id}`) : toast.error('Doctor not available')}} key={index} className='border border-blue-2 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'>
